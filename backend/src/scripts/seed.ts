...
const seedData = async () => {
  try {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const dpUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${rollNo}`;
      ...
      console.log(`  ✓ Created user ${i}/30: ${result.rows[0].name} (${result.rows[0].roll_no})`);
    }
    ...
    for (const group of groups) {
      ...
      console.log(`  ✓ Added ${memberCount} members to "${group.group_name}" (${anonymousMembers} anonymous)`);
    }

    console.log(`\n✅ Added ${totalMembers} group memberships (${anonymousMembers} anonymous, ${totalMembers - anonymousMembers} regular)\n`);

    // Create some 1-on-1 conversations with anonymous identities
    console.log('💬 Creating anonymous 1-on-1 conversations...');
    ...
    for (let i = 0; i < 15; i++) {
      ...
      // Create anonymous identity for user1
      const randomString = `anon_${Math.random().toString(36).substring(2, 15)}_${Date.now()}`;

      await pool.query(
        `INSERT INTO anonymous_identities (user_id, random_string, display_gender, target_user_id, conversation_id)
         VALUES ($1, $2, $3, $4, $5)`,
        [user1.user_id, randomString, user1.gender, user2.user_id, convResult.rows[0].conversation_id]
      );
    ...
    console.log(`\n✅ Created ${conversationCount} anonymous conversations\n`);
